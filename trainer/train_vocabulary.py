...
class TriangleTokenizationGraphConv(pl.LightningModule):
    ...
    def training_step(self, data, batch_idx):
        ...
        encoded_x = self.distribute_features_fn(encoded_x, data.faces, data.num_vertices.sum(), self.device) # 聚合顶点特征
...
def distribute_features(features, face_indices, num_vertices, device):
    ...
    return distributed_features # (3num_vertices,192)
...
@hydra.main(config_path='../config', config_name='vocabulary', version_base='1.2')
def main(config):
