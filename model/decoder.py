...
class ResNetDecoder(nn.Module):
    ...
    def __init__(self, in_feats, num_tokens, block, layers, zero_init_residual=False, width_per_group=64, ce_output=True):
        ...
        # 是否需要交叉熵输出
        if ce_output:
            self.fc = nn.Conv1d(320 * block.expansion, self.num_tokens * 14, 1)
        else:
            self.fc = nn.Conv1d(320 * block.expansion, 14, 1)

        for m in self.modules():
            if isinstance(m, nn.Conv1d): # 对卷积层使用Kaiming正态初始化
    ...
    def _forward_impl(self, x):
        ...
        x = self.layer4(x)
        x = self.fc(x)
        if self.ce_output: # permute是重新排布维度
            x = x.permute((0, 2, 1)).reshape(B, N, 14, self.num_tokens)
        else:
            x = x.permute((0, 2, 1)).reshape(B, N, 14)
        return x
...
def resnet34_decoder(in_feats, num_tokens, ce_output=False):
