    ...
    def on_validation_epoch_end(self):
        ...
        for k in range(self.config.num_val_samples):
            ...
            y = self.model.generate_with_beamsearch(
                soup_sequence, in_idx, out_idx, self.sequencer, self.config.max_val_tokens, use_kv_cache=True
            )
            print('准备生成...')
            class_indices = self.sequencer.decode(target, decoder,cluster_batches,batch)
            gaussians = self.reconstruct_from_discrete(class_indices)
            self._save_gaussian_ply(gaussians, batch.detach().cpu(),0)

            if y is None:
                continue
            
            class_indices = self.sequencer.decode(y[0], decoder,cluster_batches,batch)
            gaussians = self.reconstruct_from_discrete(class_indices)
            self._save_gaussian_ply(gaussians, batch.detach().cpu(),0)
...
def get_qsoup_model_config(config, vq_embed_levels):
    cfg = EasyDict({
        'block_size': config.block_size, # 上下文窗口
        'n_embd': config.model.n_embd, # 嵌入向量维度
        'dropout': config.model.dropout, # 正则化
        'n_layer': config.model.n_layer, # 层数
        'n_head': config.model.n_head, # 头数 
        'bias': config.model.bias, # 偏置
        'finemb_size': vq_embed_levels, # 每个Gaussian对应的tokens
        'foutemb_size': 1000, # 
    })
    return cfg
