...
from torch_geometric.data import Dataset as GeometricDataset, Batch # 图类
...
class TriangleNodes(GeometricDataset):
    ...
    def __init__(self, config, split, scale_augment, shift_augment, force_category, use_start_stop=False, only_backward_edges=False):
        ...
        self.ce_output = config.ce_output # 是否量化预测
        ...
        with open(data_path, 'rb') as fptr:
            ...
            if force_category is not None: # 仅加载此类别的数据
                for s in ['train', 'val']:
                ...
                if len(data[f'vertices_val']) == 0:
                    data[f'vertices_val'] = data[f'vertices_train'] # 存放所有坐标值
                    data[f'faces_val'] = data[f'faces_train'] # 存放各个面的顶点的坐标值的索引
                    data[f'name_val'] = data[f'name_train'] # 存放各个网格的名称
            if not config.overfit: # 是否使用完整数据集
                ...
                self.cached_vertices = data[f'vertices_train'][:1] * multiplier # 增加重复数据 *作用在列表上 直接将列表复制
    ...
    def get_all_features_for_shape(self, idx):
        ...
        if self.scale_augment: # 缩放数据增强
            if self.low_augment: # 设置范围
                x_lims = (0.9, 1.1)
                y_lims = (0.9, 1.1)
                z_lims = (0.9, 1.1)
            else:
        ...
        target = torch.from_numpy(features[:, :9]).float() # 目标
...
def create_feature_stack(vertices, faces, num_tokens):
    ...
    triangles = vertices[faces, :] # mx3x3
